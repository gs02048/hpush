...
func (server *Server) serveWebsocket(conn *websocket.Conn, tr *itime.Timer) {
	...
	for {
		...
		if p.Operation == define.OP_HEARTBEAT {
			...
			//切换房间
			oldkey := key
			if newkey, newroomid, _, err := server.changeRoom(conn, p); err == nil {
				b.Del(oldkey)
...
func (server *Server) changeRoom(conn *websocket.Conn, p *proto.Proto)(key string, rid int32, heartbeat time.Duration, err error) {
