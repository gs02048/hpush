...
func (server *Server) serveTCP(conn *net.TCPConn, rp, wp *bytes.Pool, tr *itime.Timer) {
	...
	// handshake 定时器，如果timeout时间内不成功就关闭conn
	...
	for {
		...
		}else if p.Operation == define.OP_JOIN_ROOM{
			//切换房间
			oldkey := key
			if newkey, newroomid, _, err := server.tcpChangeRoom(rr, wr, p); err == nil {
				b.Del(oldkey)
				if err = server.operator.Disconnect(oldkey, ch.RoomId); err != nil {
					log.Error("key: %s operator do disconnect error(%v)", oldkey, err)
				}
				ch.RoomId = newroomid
				b = server.Bucket(newkey)
				err = b.Put(newkey, ch)
			}
			p.Body = nil
			p.Operation = define.OP_JOIN_ROOM_REPLY
		} else {
...
// auth for goim handshake with client, use rsa & aes.
func (server *Server) tcpChangeRoom(rr *bufio.Reader, wr *bufio.Writer, p *proto.Proto) (key string, rid int32, heartbeat time.Duration, err error) {
	if p.Operation != define.OP_JOIN_ROOM {
		log.Warn("auth operation not valid: %d", p.Operation)
		err = ErrOperation
		return
	}
