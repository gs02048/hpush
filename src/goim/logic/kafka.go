...
const (
	KafkaPushsTopic = "LCS_KafkaPushsTopic"
	KafkaUserTopic = "LCS_KafkaUserTopic"
)
...
func handleSuccess() {
	...
	for {
		...
		if pm != nil {
			log.Info("producer message success, partition:%d offset:%d key:%v valus:%s topic:%s", pm.Partition, pm.Offset, pm.Key, pm.Value, KafkaPushsTopic)
		}
	}
}
...
func pushUser(userid int64,roomid,status int32,key string) (err error){
	if userid <= 0{
		return
	}
	var (
		vBytes []byte
		v      = &proto.KafkaUserMsg{OP: status, RoomId: roomid, UserId:userid,Key:key}
	)
	if vBytes, err = json.Marshal(v); err != nil {
		return
	}
	producer.Input() <- &sarama.ProducerMessage{Topic: KafkaUserTopic, Value: sarama.ByteEncoder(vBytes)}
	return
}
